    ...
    if(self)
    {
		...
		if ([self.popup.graphic.layer isKindOfClass:[AGSFeatureLayer class]]){
			...
			if (self.featureLayer.typeIdField){
				id ftVal = [self.popup.graphic.attributes objectForKey:self.featureLayer.typeIdField];
...
-(NSString *)attributeStringForField:(AGSField *)field
{
    ...
    id value = [self.popup.graphic.attributes objectForKey:field.name];
		...
		if (nTypesCount > 0){
			for (AGSFeatureType *ft in self.featureLayer.types){
				for (AGSFeatureTemplate *t in ft.templates) {
					...
					id ttv = [t.prototype.attributes objectForKey:fi.fieldName];
					if (!ttv){
						ttv = ft.typeId;
					}
